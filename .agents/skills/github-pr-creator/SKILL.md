...
レビューしやすいPR下書きを、事実ベースで素早く作成する。`git`で変更情報を収集し、`gh`でPR作成または更新まで実行できるようにする。
...
- まず`./scripts/collect_pr_context.sh`の実行を優先する:
  - `./scripts/collect_pr_context.sh --base <base-branch>`
...
- リスク領域を明示する: DB、認証/セキュリティ、API契約、設定/デプロイ、依存関係。
...
- 作成・更新は`./scripts/upsert_github_pr.sh`を優先する:
  - `./scripts/upsert_github_pr.sh --base <base-branch> --title "<title>" --body-file <file>`
